...
// ─── Component ────────────────────────────────────────────────────────────────
export default function SectionGrid({ chapter, sections }: Props) {
    ...
    <div className="min-h-screen" style={{ background: '#07070E' }}>
      ...
      <div className="max-w-[1440px] mx-auto px-4 lg:px-8 pt-12 pb-6">
        ...
        <p className="text-[15px] font-medium max-w-2xl leading-relaxed" style={{ color: '#94a3b8' }}>
          Explore all section blocks mapped to this chapter range.
        </p>

        {/* Stats row */}
        <div className="mt-8 flex flex-wrap gap-4">
          {[
            { value: sections.length, label: 'Sections', color: themeColor },
          ...
          <div className="max-w-[1440px] mx-auto flex items-center gap-4">
            ...
            <div className="relative flex-1 max-w-xl">
              ...
              <input
                ...
                placeholder="Filter by section code (A00-A09) or keyword..."
        ...
        {filtered.length === 0 && (
          ...
          >
            <p className="text-[15px] font-semibold" style={{ color: '#475569' }}>
              {sections.length === 0 ? 'No sections mapped exclusively to this chapter.' : `No sections matching "${query}"`}
            </p>
            {isSearching && (
              <button
                onClick={() => setQuery('')}
          ...
          >
            {filtered.map((c) => {
              return (
                <Link
                  key={c.id}
                  href={`/icd10cm/2026/section/${c.id}`}
